...
class HQQBaseVLLMConfig(QuantizationConfig):
    ...
    def __repr__(self) -> str:
        return (f"HQQBaseVLLMConfig(weight_bits={self.weight_bits}, "f"group_size={self.group_size})")
...
class HQQBaseVLLMLinear(LinearMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        compute_dtype = layer.scale.dtype
...
class HQQPytorchVLLMLinear(HQQBaseVLLMLinear):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        group_size = self.quant_config.group_size
        W_q        = self.unpack(layer, dtype=layer.compute_dtype).T.reshape(-1, group_size).contiguous()
    ...
    def dequantize(self, layer): #Only 8, 4, 2, 1 bit support. 3-bit NOT supported yet
        ...
        group_size = self.quant_config.group_size
        W_q        = Quantizer.unpack[self.quant_config.packing](layer.W_q, dtype=layer.compute_dtype).view(-1, group_size)
