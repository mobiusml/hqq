...
class HQQWrapper:
	...
	#####################################################
	@classmethod
	def quantize_model_(cls, model, quant_config, compute_dtype=torch.float16, device='cuda'):
		...
		cls._get_hqq_class(model).quantize_model(model, quant_config=quant_config, compute_dtype=compute_dtype, device=device)
	...
	def from_quantized(cls, save_dir_or_hub, compute_dtype=torch.float16, device='cuda', cache_dir=''):
		...
		model = cls._get_hqq_class(arch_key).from_quantized(save_dir, compute_dtype=compute_dtype, device=device, cache_dir=cache_dir)
