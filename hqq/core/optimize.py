...
def update_scale_inverse_median(W_f, scale, zero, axis, min_max):
	...
	zero_c              = zero.clone()
	zero_c_indx         = torch.sum(1.*((W_q - zero)==0), axis=axis, keepdim=True)>0
...
def update_scale_grid_search(W_f, scale, zero, axis, min_max):
	...
	rng           = torch.abs(scale).mean()*rng_dump if (rng_dump<1.) else rng_dump

	scale_shifted = torch.linspace(-rng, rng, N)[:,None].to(dtype).to(device).repeat(1, n_clusters) + scale

	#Safe inverse
	scale_shifted[torch.logical_and(scale_shifted>=0, torch.abs(scale_shifted)<=z_val)] = z_val 
	scale_shifted[torch.logical_and(scale_shifted<0,  torch.abs(scale_shifted)<=z_val)] = -z_val 
...
def optimize_weights_proximal_v2(tensor, scale, zero, min_max, axis=0, device='cuda', 
	...
	#Check
	assert lp_norm<=1., "lp_norm should be <=1"
	assert beta>0.,     "beta should be > 0"
