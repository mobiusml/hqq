...
class BaseHQQVLLMModel(BaseHQQModel):
	...
	def quantize_model_single_worker(cls, model, quant_config, compute_dtype, device):
		...
		def _patch_linear(linear_layer, quant_config):
			...
			hqq_module = HQQLinear(linear_layer, quant_config, compute_dtype=compute_dtype, device=device, del_orig=False)
		...
		cls.patch_model(model, lambda l: l.to(compute_dtype).cuda(device), _patch_linear, patch_params)

	@classmethod
	def quantize_model(cls, model, quant_config, compute_dtype=torch.float16, device='cuda'):
		workers = model.llm_engine.workers
		for i in range(len(workers)):
			cls.quantize_model_single_worker(workers[i].model, quant_config=quant_config, compute_dtype=compute_dtype, device=device)
	...
	def from_quantized_single_worker(cls, save_dir_or_hub, cache_dir='', compute_dtype=torch.float16, device='cuda:0'):
		...
		def _load_module(module, params=None):
			...
			if(('W_q' in state_dict) and ('meta' in state_dict)):
				hqq_module = HQQLinear(linear_layer=None, quant_config=None, compute_dtype=compute_dtype, device=device)
				...
				for key in state_dict:
					setattr(module, key, torch.nn.Parameter(state_dict[key].to(device), requires_grad=False))
