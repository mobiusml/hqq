...
class BaseHQQModel:
	...
	def quantize_model(cls, model, quant_config, compute_dtype=torch.float16, device='cuda'):
		...
		def _patch_linear(linear_layer, quant_config):
			return HQQLinear(linear_layer, quant_config, compute_dtype=compute_dtype, device=device) if (quant_config is not None) else linear_layer.to(compute_dtype).to(device)

		cls.patch_model(model, lambda l: l.to(compute_dtype).to(device), _patch_linear, patch_params)
